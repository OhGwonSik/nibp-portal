...
@Setter
@ToString
public class Admin618DTO {
    private Long popupZoneOid;                       // BIGINT(20) 팝업존 ID

    @NotBlank(message = "팝업존 제목은 필수 입력값입니다.")
    @Size(min = 1, max = 100, message = "팝업존 제목은 1자 이상 100자 이하로 입력해주세요.")
    private String popupZoneNm;                     // VARCHAR(100) 팝업존 제목

    @Size(max = 255, message = "요약 텍스트는 최대 255자까지 입력 가능합니다.")
    private String smry;                         // VARCHAR(255) 요약 텍스트

    @Size(max = 500, message = "링크는 최대 500자까지 입력 가능합니다.")
    private String popupZoneLink;                   // VARCHAR(500) 링크 URL
    ...
    private Long thumbnailAttachNo;                 // 썸네일 첨부파일 번호 (수정 시 imgSbstTxtCn 업데이트용)
    ...
    private List<FileDTO> popupZoneFiles;
    private List<Long> deletedAttachNos;           // 삭제할 첨부파일 번호 목록 (수정 시 사용)
